...
class TestStop:
    def test_stop_snap_only(self, mock_client):
    ...
    def test_stop_disable(self, mock_client):
        _snapd_apps.stop('lxd', disable=True)
        body = mock_client.post.call_args.kwargs['body']
        assert body['disable'] is True

    def test_stop_no_disable_default(self, mock_client):
        _snapd_apps.stop('lxd')
        body = mock_client.post.call_args.kwargs['body']
...
class TestRestart:
    def test_restart_snap_only(self, mock_client):
        _snapd_apps.restart('lxd')
        mock_client.post.assert_called_once_with(
            '/v2/apps', body={'action': 'restart', 'names': ['lxd']}
        )

    def test_restart_with_service(self, mock_client):
...
class TestListServices:
    def test_list_services_for_snap(self, mock_client):
