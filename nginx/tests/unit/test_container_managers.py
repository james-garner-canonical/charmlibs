        ...
        def __init__(self, f):
            ...
            self.nginx = Nginx(
                self.unit.get_container("nginx"),
                               NginxConfig(server_name="server",
                                           upstream_configs=[],
...
def base_state(update_cacerts):
    ...
    return scenario.State(leader=True, containers={scenario.Container("nginx", can_connect=True, execs=execs),
...
def test_layer_commands(ctx, base_state, nginx_pexp_port, nginx_insecure, nginx_port):
    ...
    assert state_out.get_container("nginx").plan.services['nginx'].command == "nginx -g 'daemon off;'"

    pexp_command = state_out.get_container("nginx-pexp").plan.services['nginx-prometheus-exporter'].command
