...
1. Delete your vendored copy of the Charmhub-hosted library, remove any references to it in your `charmcraft.yaml`, and remove any dependencies you added for it.
2. Add the library to your charm as a regular Python dependency with the appropriate version constraints -- the library's PyPI page is a good starting point here.
...
After checking out your local copy of the `charmlibs` monorepo, run the following command to add a new directory for your library with an appropriately structured Python package and tests laid out for you.
...
```
Typically this should be the `<module name>` component of the Charmhub-hosted libs name: `<charm name>.v<n>.<module name>`.
For example, `operator_libs_linux.v2.snap` is now available as `charmlibs.snap`.

However, this name must be unique across the `charmlibs` namespace packages defined in the `charmlibs` monorepo.
...
1. Is the functionality you're looking for available from the existing library with the same name?
    a. If not, would it make sense to add it to that library?
    b. If so, perhaps the library you're looking at migrating could be deprecated without migration.
...
which is just `<package name>` for general libraries.
...
3. Document in the `_<name>.py` docstring the API and patch version of the Charmhub-hosted library.
...
5. Move the contents of `PYDEPS` to the `dependencies` entry in your `pyproject.toml`, and delete the `PYDEPS` key. You should add any non-documented dependencies too (for example, `ops`). Consider adding version constraints to your dependencies too.
6. Import the public API of your library to `__init__.py` and add the imported names to `__all__`, like this:
```python
# immediately before or after from ._version -- sorted alphabetically
from ._<name> import (
...
From anywhere in the repository, run the following command:
...
From anywhere in the repository, run `just docs`.
This builds the reference docs for all the libraries.
To speed things up by only building reference docs for your library, run:
...
With any luck, your library was previously developed in a placeholder charm that exists purely for library distribution purposes.
...
The following assumes the simple case:

1. Add any unit test dependencies (like `ops[testing]`) to the `unit` dependency group in your `pyproject.toml`.
...
Currently it packs a simple `k8s` or `machine` charm (defined in the `<library path>/tests/integration/charms`) directory, depending on the `CHARMLIBS_SUBSTRATE` variable that is set in CI.
It's executed in CI via `just pack-k8s` or `just pack-machine`.
The integration tests themselves use `jubilant` to deploy and test the packed charm.
They're executed via `just integration-k8s` or `just integration-machine`.
...
You should also announce the deprecation via your team's usual communication channels.
You're free to continue to provide feature updates, but users should not expect them, and you should encourage them to migrate to get them.
