...
# format, which is private. Reusing it is deliberate -- this package and the library are
# released in lockstep and pin each other exactly, so they cannot drift, whereas a second
# copy of the wire format could. Cover every use with a test.
# from charmlibs.interfaces.example_interface import _example_interface as _internal
...
_UNIT_ID = 0
"""The simulated remote application's single unit."""

# FIXME: the databag keys this interface uses. `integrate` reads these to tell a relation
# that has never been written to from one where the conversation has already begun --
# `ops.testing` pre-populates unit databags with Juju's network keys, so an untouched
# relation is not empty.
_DATABAG_KEYS: frozenset[str] = frozenset()
...
class _Remote:
    ...
    ``get_relation`` are generic, and every charmlibs.interfaces testing package
    implements them the same way. They live here so that this package writes them once. If
    they later move to a shared dependency or into ``ops.testing`` itself, nothing visible
    to a charm author changes.
    ...
    The required properties -- immutability, readable ``endpoint`` and ``remote_app_name``,
    a useful ``__repr__`` -- are provided explicitly instead.
    """
    ...
    def _repr_args(self) -> list[str]:
        ...
        Extend this in each subclass that adds arguments, rather than overriding
        ``__repr__``::

            def _repr_args(self) -> list[str]:
                args = super()._repr_args()
                if self._thing is not None:
                    args.append(f'thing={self._thing!r}')
                return args
        """
        ...
        *,
        ...
        Raises:
            ...
            ValueError: If this remote already has a relation with data on it in ``state``,
                if ``end`` isn't one of the three values, or if adding the relation would
                exceed what the library supports on one endpoint.
        """
    ...
    def _check_can_add(self, state: testing.State) -> None:
        """Raise if adding a relation for this remote isn't supported.

        Supporting several remotes on one endpoint is optional, but silently replacing the
        first one is not: a library that doesn't support it must raise ``ValueError``.
        Override this where the library can't support it -- for example because its
        implementation reads the relation with ``Model.get_relation``, which raises when an
        endpoint carries more than one::
            ...
            def _check_can_add(self, state: testing.State) -> None:
                ...
                if existing:
                    ...
                    raise ValueError(
                        f'Endpoint {self.endpoint!r} already has a relation (to {others}), '
                        'and this library supports only one remote per endpoint. Use a '
                        'second endpoint, with its own remote, instead.'
                    )
        """
...
class RemoteProvider(_Remote):
    """A simulated example-interface provider, for testing a **requirer** charm.

    The remote plays the opposite role to the charm under test, so a requirer charm is
    tested with a ``RemoteProvider``.
    ...
    The happy path is one line::
        ...
        def test_the_happy_path(ctx: testing.Context):
            with example_interface_testing.mocked():
                state = REMOTE.integrate(ctx, testing.State.from_context(ctx))
    ...
    defaulting to the happy-path behaviour of a well-behaved provider. Document each one
    here, and expose it as a read-only property so that instances stay immutable.
    """

    def publish(self, state: testing.State) -> testing.State:
        """Write the provider's data for this state.

        The charm is not executed.

        FIXME: implement this. It is the only method that needs to know the interface, and
        its contract is:

        - **Recompute, don't append.** Add what the charm's published data now warrants,
          keep what is still warranted, and *remove what is no longer warranted* -- an
          answer to a request the charm has since withdrawn, say, which is what the real
          remote charm's library does. The postcondition is "the remote's data is correct
          for this state", not "an answer has been appended". It follows that ``publish`` is
          idempotent: calling it twice with nothing else changed leaves the state unchanged.
        - **Derive the answer.** Where the charm under test writes before the remote does,
          this remote's data must be derived from what the charm *actually published*, read
          off ``relation.local_app_data`` and ``relation.local_unit_data``, never from a
          canned value. Ignoring the charm's data and writing fixed values satisfies every
          other clause while reintroducing exactly the silent mismatches this package exists
          to prevent. Where the remote writes first there is nothing to derive from, and the
          data is supplied as ordinary fixture configuration.
        - **Write everything the remote owns**, not only relation data: ``Secret`` objects
          it owns or grants, and anything else the library expects to find in the state.
        - **Don't raise on an absence of data.** Where the charm has published nothing to
          answer, or where the remote never writes on this interface at all, write nothing
          and return the state unchanged -- so that a test arranging this relation
          incidentally, while being about something else, isn't obstructed. A missing
          *relation* is different: that is an incoherent call, and ``_relation_for`` raises
          for it. Distinguish likewise between what the interface *cannot express*, which
          must raise ``ValueError``, and what the remote simply *would not do* in this
          state, which is a no-op. Where there is a specific, unambiguous reason the charm
          published nothing -- a library that writes to the application databag being used
          with a non-leader state, say -- raise and say so, or log a warning where the
          reason is likely but not certain.
        - **Preserve the rest of the state.** Only this remote's own relation data and
          library-managed state may be touched. Other remotes on the same endpoint, other
          relations, containers and config are preserved as they are.
        ...
        # FIXME: read what the charm published from relation.local_app_data and
        # relation.local_unit_data, and derive this remote's data from it. Writing the
        # databags back unchanged is a no-op placeholder, so that every other method works
        # and the tests below have something to run against.
        return _replace_relation(
            state,
            relation,
            remote_app_data=dict(relation.remote_app_data),
            remote_units_data={
...
class RemoteRequirer(_Remote):
    """A simulated example-interface requirer, for testing a **provider** charm.

    The remote plays the opposite role to the charm under test, so a provider charm is
    tested with a ``RemoteRequirer``.

    The happy path is one line::

        REMOTE = example_interface_testing.RemoteRequirer('endpoint')
    ...
    each defaulting to the happy-path behaviour of a well-behaved requirer. Document each
    one here, and expose it as a read-only property so that instances stay immutable.
    """

    def publish(self, state: testing.State) -> testing.State:
        """Write the requirer's data for this state.

        The charm is not executed. See :meth:`RemoteProvider.publish` for the full contract
        this must satisfy.
        ...
        _require_mocked('publish')
        relation = self._relation_for('publish', state)
        # FIXME: as for RemoteProvider.publish. Where the requirer writes first -- the usual
        # case for a request-response interface -- there is nothing to derive from, so this
        # remote's requests come from its constructor arguments instead.
        return _replace_relation(
            state,
            relation,
            remote_app_data=dict(relation.remote_app_data),
