...
class OAuthRequirerCharm(CharmBase):
    def __init__(self, *args: Any) -> None:
        super().__init__(*args)
        self.config_to_update: ClientConfig | None = None
    ...
    @property
    def client_secret_content(self) -> dict[str, str]:
...
def dict_to_relation_data(dic: dict[str, Any]) -> dict[str, str]:
    return {k: json.dumps(v) if isinstance(v, (list, dict)) else str(v) for k, v in dic.items()}


def test_data_in_relation_bag(context: Context[OAuthRequirerCharm]) -> None:
...
def test_no_event_emitted_when_provider_info_available_but_no_client_id(
    context: Context[OAuthRequirerCharm], provider_info: dict[str, Any]
) -> None:
...
def test_get_client_credentials_when_data_available(
    context: Context[OAuthRequirerCharm], provider_info: dict[str, Any]
) -> None:
    client_id = 'client_id'
    client_secret = 's3cR#T'
...
def test_warning_when_http_redirect_url(
    context: Context[OAuthRequirerCharm], caplog: pytest.LogCaptureFixture
) -> None:
...
def test_redirect_uri_optional_without_authorization_code_grant(
    context: Context[OAuthRequirerCharm],
) -> None:
    relation = Relation('oauth')
    state = create_state(leader=True, relations=[relation], containers=[])
...
def test_exception_raised_when_invalid_grant_type(context: Context[OAuthRequirerCharm]) -> None:
    state = create_state(leader=True, containers=[])
...
def test_exception_raised_when_invalid_client_authn_method(
    context: Context[OAuthRequirerCharm],
) -> None:
    state = create_state(leader=True, containers=[])
...
def test_event_emitted_when_invalid_client_config(
    context_invalid_config: Context[InvalidConfigOAuthRequirerCharm],
) -> None:
    relation = Relation('oauth')
    state = create_state(leader=True, relations=[relation], containers=[])
