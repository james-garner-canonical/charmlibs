...
- `capabilities` (`relation_for_requirer` only) — a `ProviderCapabilities` the simulated provider has advertised. Left out, the provider hasn't advertised and `get_provider_capabilities()` returns `None` ("not known yet"); the capability-aware callable form of the library's `certificate_requests` is exercised the same way. A provider charm under test advertises its own capabilities, so `relation_for_provider` needs nothing here.
- `response=False` — populate only the requirer's side, for a request the provider hasn't answered yet (`capabilities` are still advertised if given).
