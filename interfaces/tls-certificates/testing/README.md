...
- `certificate_requests` — the requests the requirer has made. Each entry is a bare `CertificateRequestAttributes` (answered with a certificate), or wrapped to describe that request's outcome: `denied(...)` for an error instead of a certificate, `renewing(...)` for a certificate late enough in its validity period that the library re-requests it on the next reconcile, `expired(...)` for one already dead, or `revoked(...)` for one the provider has since revoked. A request with `is_ca=True` is answered with a CA certificate, and every issued certificate is published with its real chain (leaf to root). Mixed outcomes in one relation are fine, and every request appears on the requirer's side regardless of outcome — the charm asked, whatever the provider did about it. To make *all* certificates stale, wrap each request: `[renewing(r) for r in REQUESTS]`; `renewing` needs no arguments beyond the request, whatever `renewal_relative_time` your charm uses. The type of an entry — bare or wrapped — is `CertificateRequest`, for annotating mixed lists.
...
Note which renewal that exercises. The library renews two ways: it schedules a Juju secret expiry when it stores a certificate, and `secret-expired` drives the renewal; and a safety net re-checks every certificate on every reconcile, for when that event doesn't fire or doesn't complete. `renewing(...)` reaches the safety net only. The secret is created by the library *as the charm runs*, so a state built beforehand holds none and there is nothing to fire `ctx.on.secret_expired` at. Both routes withdraw the request and re-request, so the charm ends up in the same place — but if you want to exercise the secret-expiry route specifically, run the charm until it holds certificates and fire the event at the secret it created. There is no helper for finding that secret yet.

To test the opposite case — renewal *not* happening — use `expired(...)`. The library's safety net covers certificates approaching expiry and stops at expiry itself, so a dead certificate stays assigned and is never re-requested. What the charm does about that is the charm's decision, and `expired(...)` is how to pin it down.

There is no counterpart for a **provider** charm under test. `respond_to_requests` answers requests on a relation's local side, and passing it a relation from `relation_for_provider` raises rather than discarding the requests on that relation's remote side. A provider charm issues its own certificates when it runs, so it needs no help answering; what has no helper yet is the *other* direction — playing the simulated requirer's next move, so that a provider charm can be tested against a requirer that rotates its key or withdraws a request mid-test. Unlike the requirer case there is no charm to derive that move from, so today it means building a fresh `relation_for_provider` with the new requests, which loses the certificates the charm under test had already issued.
