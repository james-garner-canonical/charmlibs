...
def _renewal_safety_threshold(renewal_relative_time: float) -> float:
    ...
    expiry. The testing package derives its back-dated certificates from this same
    function, so that a change here can't leave its ``renewing`` fixture quietly failing
...
class TLSCertificatesRequiresV4(Object):
    ...
    def _generate_private_key(self, mode: Literal[Mode.UNIT, Mode.APP]) -> None:
        ...
        """
        # PrivateKey.generate rather than the module-level generate_private_key, which is
        # deprecated and so emitted a DeprecationWarning from the library's own code path --
        # a warning the charm author could do nothing about.
        self._store_private_key_in_secret(PrivateKey.generate(), mode)
