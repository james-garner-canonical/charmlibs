...
def mocked() -> Iterator[None]:
    ...
    reading and writing relation data, which ``ops.testing`` already models, with no
    side effects and nothing slow enough to be worth replacing. The scope is defined, and
    required, all the same -- a library that didn't require it would break every test
    written against it on the day it started mocking something, whereas requiring it from
    the start makes introducing mocking a non-breaking change.
    """
...
def require_mocked(method: str) -> None:
    ...
    if _depth():
        return
    raise RuntimeError(
        f'{method} must be called inside a tracing_testing.mocked() scope, which has to be '
        'entered before the charm runs:\n\n'
        '    with tracing_testing.mocked():\n'
